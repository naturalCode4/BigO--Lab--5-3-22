...
// ANSWER: O(n). O(1) if you assume you won't change the numbers array
...
// ANSWER: No. I could write code that's very slightly more efficient so it doesnt have to look at numbers[0]-numbers[2], but its still O(1)
...
// ANSWER: O(1). If we're going to add to numbers, then O(n), as grows linearly with arr size
...
// Q: What is the time complexity of the function? 
// ANSWER: O(n) because it still has to load whole array in? If not, then O(1)
...
// ANSWER: Yes by not creating new space for a new numbers array, and just squaring itself
...
// ANSWER: Yes. The second function has two iteractions through the string, whereas the first has only 1. Writing fancy and elegant code does not make you a more efficient coder
